...
import {
  ...
  parseObject,
  parseNodeFlags,
} from '@bablr/agast-helpers/parsers';
...
export const parseCallable = (input) => {
  ...
  let reference;
  if (canStartIdentifier(chr) || '_#.'.includes(chr)) {
...
export const parseMatcher = (input) => {
  ...
  switch (chr) {
    case '/':
      ...
      return buildEmbeddedRegexMatcher([...str.slice(startIdx, endIdx)].join(''));
    case '"':
    case "'":
...
export const parseTreeNodeMatcher = (input) => {
  ...
  if (`'"`.includes(chr)) {
    ...
    literalValue = buildEmbeddedRegexMatcher([...str.slice(startIdx, endIdx)].join(''));
  }
