...
export const buildChildren = (children) => {
  ...
  for (const child of children) {
    ...
    while (value.type === Symbol.for('BindingExpression')) {
      let bindingTag = reifyExpression(get('binding', value));
      bindingTags.push(bindingTag);
      bindings.push(bindingTag.value);
    ...
    let property = buildProperty(referenceTag.value, bindings, value);
    built = Tags.push(
      built,
      buildChild(
        PropertyWrapper,
...
export const reifyExpression = (node) => {
  ...
  switch (node.type?.description || node.type) {
    ...
    case 'PropertyMatcher': {
      ...
      return { refMatcher, bindingMatchers, nodeMatcher };
    }
    ...
    case 'BoundNodeMatcher': {
      ...
      return { bindingMatchers, nodeMatcher };
    }
