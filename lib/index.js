...
import { buildBindingSegment, buildEmbeddedMatcher } from './builders.js';
import {
  AttributeDefinition,
  CloseNodeTag,
  GapTag,
...
export const reifyBablrOptions = (str) => {
  ...
  return freezeRecord({ literal, shift, bind, allowEmpty, internal, hold });
};
...
export const buildChildren = (children) => {
  ...
  for (const child of children) {
    if ([AttributeDefinition, LiteralTag].includes(child.value.name)) {
      built = Tags.push(reifyExpression(child), built);
      continue;
    }
    ...
    // let bindings = bindingTags.map((tag) => parseTag(tag).value);

    let referenceTag = reference && !isNullNode(reference) ? reifyExpression(reference) : null;
    ...
    built = Tags.push(
      buildPropertyTag(
        Tags.fromValues(
          [printTag(referenceTag) || Tags.fromValues([]), Tags.fromValues(bindingTags), node],
          1,
        ),
      ),
      built,
  ...
  switch (node.value.name?.description) {
    ...
    case 'TreeNode': {
      ...
      if (literalValue && !flags.token) {
        let tokenFragment = buildNode(
          Tags.fromValues([
            buildOpenNodeTag(tokenFlags, null, literalValue, freezeRecord({}), true),
          ]),
        );
        ...
        return buildNode(
          Tags.fromValues([
            ...
            Tags.fromValues([
              buildPropertyTag(Tags.fromValues(['_:', Tags.fromValues([]), tokenFragment])),
            ]),
            buildCloseNodeTag(),
          ]),
        );
    ...
    case 'IdentifierPath': {
      return deepFreezeRecord(
        [...list('segments', node)].map((segment) => reifyExpression(segment)),
      );
    }
    ...
    case 'OpenNodeTag': {
      ...
      if (!openToken) {
        return buildOpenNodeTag(tokenFlags, null, literalValue, freezeRecord({}), selfClosing);
      }
      ...
      attributes = reifyExpression(attributes) || freezeRecord({});
    ...
    case 'PropertyMatcher': {
      ...
      return deepFreezeRecord({ refMatcher, bindingMatchers, nodeMatcher });
    }
    ...
    case 'ReferenceMatcher': {
      ...
      return freezeRecord({ type, name, flags });
    }
    ...
    case 'BoundNodeMatcher': {
      ...
      return freezeRecord({ bindingMatchers, nodeMatcher });
    }
    ...
    case 'BoundNode': {
      ...
      return freezeRecord({ bindingTags, node: node_ });
    }
    ...
    case 'BindingSegment': {
      ...
      switch (path.value.name && path.value.name.description) {
        case 'Identifier':
          return buildBindingSegment(null, reifyExpression(path));
        case null:
          return buildBindingSegment(reifyExpression(path), null);
        default:
          throw new Error();
      }
    }
    ...
    case 'BindingMatcher': {
      ...
      return deepFreezeRecord({ segments });
    }
    ...
    case 'Call': {
      ...
      return deepFreezeRecord({ verb: reifyExpression(verb), arguments: args });
    }
    ...
    case 'Array': {
      return deepFreezeRecord([...list('elements', node)].map((el) => reifyExpression(el)));
    }
