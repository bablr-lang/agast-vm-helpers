...
export const reifyNodeFlags = (flags) => {
  let { token, hasGap, fragment, cover } = flags.value.open.attributes;
...
export const buildChildren = (children) => {
  ...
  for (const child of children) {
    if ([AttributeDefinition, LiteralTag].includes(child.value.open.type)) {
      built = Tags.push(built, reifyExpression(child));
      continue;
    }

    if (child.value.open.type !== Symbol.for('Property')) throw new Error('umimplemented');
    ...
    if (node.value.open.flags.fragment && !node.value.open.flags.cover) {
      for (let child of Tags.traverse(node.tags)) {
        if (![OpenNodeTag, CloseNodeTag].includes(child.value.open.type))
          built = Tags.push(built, child);
      }
      continue;
    }

    if (node.value.open.type === NullTag || node.value.open.type === GapTag) {
    ...
    built = Tags.push(
      built,
      buildChild(Property, buildProperty([referenceTag, bindingTags, node])),
    );
  }
...
export const reifyExpression = (node) => {
  ...
  if (getFlags(node)?.token && !node.value.open.type) {
    return getCooked(node);
  }

  switch (node.value.open.type?.description) {
    case 'Document': {
    ...
    case 'TreeNode': {
      ...
      if (literalValue && !flags.token) {
        let tokenFragment = buildNode(
          Tags.fromValues([buildOpenNodeTag(tokenFragmentFlags, null, literalValue, {}, true)]),
        );

        return buildNode(
          Tags.fromValues([
            buildOpenNodeTag(flags, type, null, attributes),
            Tags.fromValues([buildProperty([buildReference(), [], tokenFragment])]),
            buildCloseNodeTag(),
          ]),
        );
      } else {
    ...
    case 'ReferenceTag': {
      let { isArray } = node.value.open.attributes;
    ...
    case 'TreeNodeMatcherOpen': {
      ...
      type = type
        ? type.value.open.type === Symbol.for('String')
    ...
    case 'PropertyMatcher': {
      ...
      if (boundMatcher.value.open.type !== Symbol.for('BoundNodeMatcher')) throw new Error();
    ...
    case 'BindingSegment': {
      ...
      switch (path.value.open.type && path.value.open.type.description) {
        case 'Identifier':
