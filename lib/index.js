...
export const reifyNodeFlags = (flags) => {
  let { token, hasGap, fragment, cover } = flags.value.attributes;
...
export const buildChildren = (children) => {
  ...
  for (const child of children) {
    if ([AttributeDefinition, LiteralTag].includes(child.value.type)) {
      built = Tags.push(built, reifyExpression(child));
      continue;
    }

    if (child.value.type !== Symbol.for('Property')) throw new Error('umimplemented');
    ...
    if (node.value.flags.fragment && !node.value.flags.cover) {
    ...
    if (node.value.type === NullTag || node.value.type === GapTag) {
...
export const reifyExpression = (node) => {
  ...
  if (getFlags(node)?.token && !node.value.type) {
    return getCooked(node);
  }

  switch (node.value.type?.description) {
    case 'Document': {
    ...
    case 'ReferenceTag': {
      let { isArray } = node.value.attributes;
    ...
    case 'TreeNodeMatcherOpen': {
      ...
      type = type
        ? type.value.type === Symbol.for('String')
    ...
    case 'PropertyMatcher': {
      ...
      if (boundMatcher.value.type !== Symbol.for('BoundNodeMatcher')) throw new Error();
    ...
    case 'BindingSegment': {
      ...
      switch (path.value.type && path.value.type.description) {
        case 'Identifier':
