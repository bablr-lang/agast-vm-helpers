...
export const buildChildren = (node) => {
  ...
  if (selfClosing) {
    ...
    for (const child of sumtree.traverse(children.node)) {
      ...
      let referenceTag = reifyExpression(reference.node);

      built = sumtree.push(built, referenceTag);
      built = sumtree.push(
        built,
        buildProperty(referenceTag.value, buildBindingTag().value, reifyExpression(value.node)),
      );
    }
...
export const reifyExpression = (node) => {
  ...
  if (node.language === 'https://bablr.org/languages/core/en/cstml') {
    switch (node.type?.description || node.type) {
      ...
      case 'Type': {
        let { language, name } = node.properties;
      ...
      case 'DoctypeTag': {
        ...
        return buildDoctypeTag(
          reifyExpression(attributes.node),
      ...
      case 'LiteralTag': {
        ...
        return buildLiteralTag(getCooked(value.properties.content));
      }
      ...
      case 'OpenNodeTag': {
        ...
        let language = type?.language;

        return {
          type: OpenNodeTag,
          value: { flags, language, type: type?.name, attributes },
        };
      }
  ...
  switch (printType(node.type)) {
    ...
    case 'UnionTypeMatcher': {
      const { elements = [] } = node.properties;

      return [...btree.traverse(elements)].map((el) => reifyExpression(el.node));
    }

    case 'LiteralTypeMatcher': {
      let { language, name } = node.properties;

      language = language && reifyExpression(language.node);
      name = name && reifyExpression(name.node);
    ...
    default:
      throw new Error(`bad expression type: ${printType(node.type)}`);
  }
};
...
export const reifyExpressionShallow = (node) => {
  ...
  switch (printType(node.type)) {
    ...
    case 'Object': {
      ...
      return Object.fromEntries(
        [...btree.traverse(properties.node)].map(
          ({
            node: {
              properties: { key, value },
            },
    ...
    case 'Array':
      return [...btree.traverse(node.properties.elements.node)].map((prop) => prop.node);
