...
export const reifyExpression = (node) => {
  ...
  switch (node.type?.description || node.type) {
    ...
    case 'IdentifierPath': {
      return [...list('segments', node)].map((segment) => {
        switch (segment.type.description) {
          case 'Identifier':
            return freeze({ type: null, name: reifyExpression(segment) });
          case 'Punctuator':
            return freeze({ type: reifyExpression(segment), name: null });
          default:
            throw new Error();
        }
      });
    ...
    case 'BindingTag':
      ...
      return buildBindingTag(languagePath ? reifyExpression(languagePath) : []);
