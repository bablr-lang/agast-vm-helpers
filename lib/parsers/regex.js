...
import { arrayIncludes, freezeRecord } from '@bablr/agast-helpers/object';
import { parseUnsignedInteger } from '@bablr/agast-helpers/parsers';
...
export const parseRegexPattern = (input) => {
  ...
  let expression = freezeRecord({ capture: true, name: null, alternatives });

  return freezeRecord({ expression, flags });
};
...
export const parseAlternatives = (input) => {
  ...
  return freezeRecord(alternatives);
};
...
export const parseElements = (input) => {
  ...
  while (chr && chr !== '|' && chr !== '/' && chr !== ')') {
    ...
    if (arrayIncludes(quantifierChrs, chr)) {
      let min, max, greedy;
...
export const parseGroup = (input) => {
  ...
  return freezeRecord({ capture, alternatives });
};
