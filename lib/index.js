...
export const reifyNodeFlags = (flags) => {
  ...
  let token = !!(tokenToken && reifyExpression(tokenToken.node));
  let hasGap = !!(hasGapToken && reifyExpression(hasGapToken.node));
  let fragment = !!(fragmentToken && reifyExpression(fragmentToken.node));
  let cover = !!(fragment && !(multiFragmentToken && reifyExpression(multiFragmentToken.node)));
  ...
  for (const property of btree.traverse(properties.node)) {
    switch (property.node.type) {
      case Symbol.for('Property'): {
        let { reference, binding, value: node } = property.node.properties;

        reference = (reference ? reifyExpression(reference.node) : buildReferenceTag('.')).value;
        binding = (binding ? reifyExpression(binding.node) : buildBindingTag()).value;
...
export const reifyExpression = (node) => {
  ...
  switch (node.type?.description || node.type) {
    ...
    case 'AttributeDefinition': {
      let { path, value } = node.properties;
    ...
    case 'Object': {
      ...
      return Object.fromEntries(
        [...btree.traverse(properties.node)].map((property) => {
          ...
          return [reifyExpression(key.node), reifyExpression(value.node)];
        }),
      );
    }
